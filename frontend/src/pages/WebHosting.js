...
const WebHosting = () => {
  ...
  const handleAddToCart = async (plan) => {
    try {
      ...
      await addHosting(plan._id)
      toast.success(`${plan.name} plan added to cart`)
      navigate('/cart')
    } catch (err) {
      console.error('Error adding hosting plan:', err)
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      try {
        const response = await fetch(`${API_URL}/hosting/plans`)
        const data = await response.json()

        if (!response.ok) {
          setError(data.error || 'Failed to fetch hosting plans')
        } else {
          setPlans(data)
        }
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-primary mt-16 pb-20 px-4 sm:px-6">
        ...
        <div className="flex items-center justify-center gap-3 mb-8">
          <div className="text-center py-6">
            ...
            <p className="text-lg text-center text-gray-600 max-w-3xl">
              Get a powerful web hosting platform for your website at affordable
              prices.
            </p>
          </div>
        </div>
        ...
        {loading && (
          <div className="max-w-2xl mx-auto p-6 my-40">
            <div className="flex justify-center items-center py-12">
              ...
              <span className="ml-4 text-lg">Loading Hosting plans...</span>
            </div>
          </div>
        )}
          ...
          {plans.map((plan) => (
            <div
              ...
              <div className="p-6">
                ...
                <p className="text-text text-center text-sm mb-8">
                  {plan.description}
                </p>
                  ...
                  {Object.entries(plan.resources).map(([key, value]) => {
                    ...
                    return (
                      <div
                        ...
                        <span className="text-gray-600 ml-auto">{value}</span>
                      </div>
                    )
                  })}
                </div>
